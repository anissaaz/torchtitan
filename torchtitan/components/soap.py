            ...
            if merge_dims:
                ...
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        ...
                        # for torchtitan DTensor
                        if isinstance(outer_product, DTensor):
                            outer_product = outer_product.to_local()
    ...
    def get_orthogonal_matrix(self, mat):
        ...
        for m in matrix:
            if len(m) == 0:
                ...
                continue
            original_device = m.data.device
            # Move to CPU for eigh — avoids large float64 allocation on GPU
            m_cpu = m.cpu().float()
            try:
                _, Q = torch.linalg.eigh(m_cpu+1e-30*torch.eye(m_cpu.shape[0]))
            except:
                _, Q = torch.linalg.eigh(m_cpu.double() + 1e-30 * torch.eye(m_cpu.shape[0]))
                Q = Q.to(m.dtype)
            Q = torch.flip(Q, [1])
